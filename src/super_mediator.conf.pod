...
information elements to be present to use particular features.  The syntax of
collectors is described in L</Collector Block> below.
...
collectors.  See L</Filter Block>.

=item exporter

An exporter specifies a destination for the flow data it processes.
...
while the connection is down.  Exporters are described in L</Exporter Block>
below.
...
defined.  See L</Group Block>

=item dns-dedup

B<super_mediator> will perform dns de-duplication for each exporter that
...
specified in the DNS_DEDUP block.  See L</DNS De-duplication Block>
...
This block is optional; its syntax is described in L</DPI Configuration Block>.
...
output files.  See L</MULTI_FILES> below for more information.
...
disabled unless explicitly specified within one of the lists.  See
L</SSL_CONFIG Block>.
...
seen in the configurable timeout period. See L</DEDUP_CONFIG Block>.
...
be included in the output record.  The syntax is described in L</MAP Blocks>.
...
dns-dedup, dpi-config, ssl-config, dedup-config, or map block, the top-level
context
...
any previous observationDomainId value in the records.  If the
B<PRESERVE_OBDOMAIN> keyword is present anywhere in the configuration file
outside of a collector, filter, exporter, or group block, B<super_mediator>
preserves the incoming records' value of the observationDomainId.  If the
incoming records do not contain that element, the records exported by
B<super_mediator> contain an observation domain of zero.

=item B<REWRITE_SSL_CERTS>

If specified, B<super_mediator> will, for all IPFIX exporters, rewrite the SSL
certificate records that are represented in a subTemplateList on the SSL DPI
record (template 51722 (0xca0a)).  The rewriting changes the structure of
template 51723 (0xca0b) and eliminates template 52756 (0xce14).  The new SSL
certificate record has explicit information elements for parts of the
...
The log level can be specified by the B<LOGLEVEL> keyword.  Default level is
WARNING.
...
If present, and B<super_mediator> is run in daemon mode, B<super_mediator>
...
it will not receive the necessary templates until B<yaf(1)> periodically
transmits
...
is I<TCP> or I<UDP>.  If not specified, the default is 18000.
...
This command specifies the time (seconds) between directory polls if the
collector
...
If present, require a record to pass all filters in the COLLECTOR block to
process the record.  In effect, join the filters with a logical B<AND>.

=item B<Collector Filters>

Each collector can contain one or more filters to define what data should
be collected.  The syntax for the filters is the same as
defined below in the L</Filter Block>.  The filters defined in the collector
block
are only for the collector that contains them.  The filters are by default
are treated as B<OR> filters: a record is processed if it passes I<ANY> filter.
Use B<AND_FILTER> to make the filters AND filters. If a filter
...
flows on collection.  One and only filter block can be specified in the
...
By default, if B<any> comparison in a filter returns a match or success,
the flow record
...
transport protocol, and thus cannot guarantee delivery of messages.  The
...
The exporter will write delimited text to a file or JSON if the keyword JSON is
present
...
included program, B<super_table_creator>, which is available if mysql is
installed, you can create a MySQL table for the full flow, by running
B<super_table_creator> with C<--flow-only>.
...
is the object identifier as given in the X.509 ASN.1 RelativeDistinguishedName
sequence. A list of common identifiers are listed below with the element ID
numbers.
...
interval, in seconds, that the resource record may be cached for.
...
Process. This is typically set on the B<yaf(1)> command line.
...
information is from the Question Section, 1 for the Answer Section, 2 for the
Name Server Section, and 3 for the Additional Section.
...
Flow start time in ISO 8601 format, with milliseconds (YYYY-MM-DD
hh:mm:ss.ssss).
...
Forward first-packet TCP flags where each flags bit is represented by the
first character in the flag's name: B<F>IN, B<S>YN, B<R>ST, B<P>SH, B<A>CK,
B<U>RG, B<E>CE, B<C>WR.

=item RIFLAGS,riflags,30

Reverse first-packet TCP flags where each flags bit is represented by the
first character in the flag's name: B<F>IN, B<S>YN, B<R>ST, B<P>SH, B<A>CK,
B<U>RG, B<E>CE, B<C>WR.

=item UFLAGS,uflags,31

Forward I<n>th-packet TCP flags union where each flags bit is represented by
the first character in the flag's name: B<F>IN, B<S>YN, B<R>ST, B<P>SH,
B<A>CK, B<U>RG, B<E>CE, B<C>WR.

=item RUFLAGS,ruflags,32

Reverse I<n>th-packet TCP flags union where each flags bit is represented by
the first character in the flag's name: B<F>IN, B<S>YN, B<R>ST, B<P>SH,
B<A>CK, B<U>RG, B<E>CE, B<C>WR.
...
averageInterarrivalTime field in flow statistics in fractional seconds. See
yaf(1).

=item RITIME,ritime,57

reverseAverageInterarrivalTime field in flow statistics in fractional
seconds. See yaf(1).
...
firstEightNonEmptyPacketDirections field in flow statistics in hexadecimal
format. See yaf(1).
...
The maximum segment size reported in the Maximum Segment Size TCP Option. This
should
be consistent over all subflows. See yaf(1).

=item MPTCPID,mptcpid,86

The address ID of the subflow found in the SYN/ACK of an MP_JOIN operation. See
yaf(1)
...
The initial n bytes of forward direction of the flow's payload.  Present if
B<yaf>
...
The initial n bytes of reverse direction of the flow's payload. Present if
B<yaf>
...
A comma separated list of DHCP Options in the order they were requested. See
yafdhcp(1)

=item RDHCPOPTIONS,rdhcpoptions,91

The comma separated list of DHCP Options in the reverse flow as they were
requested. See yafdhcp(1)
...
The NDPI Layer 7 Sub/secondary protocol identified by the nDPI library via
yaf(1)
...
information elements contained in <DPI_IE_LIST>.  The I<DPI_IE_LIST> is a list
of information element ids from the below list (see DPI_CONFIG), separated by a
...
metadata in ipfix output.  This option is not available to TEXT exporters.
...
By default, the B<super_mediator> includes the timestamp (flow end time) of the
first flow
...
successfully uploaded.  B<super_mediator> does not consider MySQL warnings
...
If present, <super_mediator> will perform SSL certificate de-duplication on SSL
certificates received from B<yaf(1)>.  It will also write previously
...
contained in I<DNS_RESOURCE_RECORD_TYPE_LIST>.  The
I<DNS_RESOURCE_RECORD_TYPE_LIST>
is a list of resource record types, separated by a comma, and surrounded by
square brackets, [ and ].  For example, the following line will direct
B<super_mediator> to only dedup on A Records and NS Records. Resource record
types available are 0 for
...
B<super_mediator> to determine which SSL/TLS X.509 certificates to export
...
By default, B<super_mediator> hashes the SSL Certificate Serial Number and
Issuer Name to store and aggregate SSL Certificates.  In addition to the serial
number, B<super_mediator>
can aggregate the data per VLAN ID (or group of VLAN IDs) or per Observation
Domain ID (or group of Observation domains) with a VLAN_MAP or OBID_MAP.
...
This field is populated if the DEDUP_CONFIG block was configured with
B<ADD_EXPORTER_NAME> or the PREFIX line contained a B<VLAN_MAP> or B<OBID_MAP>.
...
The common name of the Issuer (Certificate Authority) in the X.509 Certificate
(first certificate in the chain). Only present for SSL de-duplicated data.
...
responses.  244 will de-duplicate SSL certificate chains used by a particular
IP.
...
If present, the B<super_mediator> will flush a dedup record from the cache when
a new record has not been seen for over I<FLUSH_SECONDS>.  B<super_mediator>
will write the record when either the B<FLUSH_TIME> or B<MAX_HIT_COUNT>
condition has been met. The default I<FLUSH_SECONDS> is 300 seconds, or 5
minutes.
...
=item B<TOMBSTONE_CONFIGURED_ID> I<TOMBSTONE_CONFIGURED_ID>

If present, the B<TOMBSTONE_CONFIGURED_ID> command will override the default id
value of 0 with the given I<TOMBSTONE_CONFIGURED_ID>. This value must be
